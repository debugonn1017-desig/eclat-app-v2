-- v0.3.68: 月〜土の曜日別来店実績順
--
-- 目的:
--   - v0.3.67 の直近10回の実来店定義を変えず、曜日ごとの回数と最新日を保持する
--   - 顧客検索では選択曜日の回数→その曜日の最新来店日の順で全件ソート後にページングする
--
-- 注意:
--   - 予定(is_planned=true)は除外する
--   - 0円の実来店は含める
--   - 既存列の名前・型・順序を維持し、新しい列は末尾だけに追加する
--   - SECURITY INVOKER により既存 customer_visits / customers RLS を維持する
...
select
  ...
  coalesce(metrics.weekday_1_count, 0) as weekday_1_count,
  metrics.weekday_1_last_visit_date,
  coalesce(metrics.weekday_2_count, 0) as weekday_2_count,
  metrics.weekday_2_last_visit_date,
  coalesce(metrics.weekday_3_count, 0) as weekday_3_count,
  metrics.weekday_3_last_visit_date,
  coalesce(metrics.weekday_4_count, 0) as weekday_4_count,
  metrics.weekday_4_last_visit_date,
  coalesce(metrics.weekday_5_count, 0) as weekday_5_count,
  metrics.weekday_5_last_visit_date,
  coalesce(metrics.weekday_6_count, 0) as weekday_6_count,
  metrics.weekday_6_last_visit_date
from sample_counts counts
left join weekday_arrays weekdays on weekdays.customer_id = counts.customer_id
left join weekday_metrics metrics on metrics.customer_id = counts.customer_id
...
-- v0.3.58〜v0.3.67 の既存列・既存ロジックをそのまま維持し、
-- 曜日別ソート列だけを末尾へ追加する。
...
select
  ...
  coalesce(pattern.weekday_1_count, 0) as metric_pattern_weekday_1_count,
  pattern.weekday_1_last_visit_date as metric_pattern_weekday_1_last_visit_date,
  coalesce(pattern.weekday_2_count, 0) as metric_pattern_weekday_2_count,
  pattern.weekday_2_last_visit_date as metric_pattern_weekday_2_last_visit_date,
  coalesce(pattern.weekday_3_count, 0) as metric_pattern_weekday_3_count,
  pattern.weekday_3_last_visit_date as metric_pattern_weekday_3_last_visit_date,
  coalesce(pattern.weekday_4_count, 0) as metric_pattern_weekday_4_count,
  pattern.weekday_4_last_visit_date as metric_pattern_weekday_4_last_visit_date,
  coalesce(pattern.weekday_5_count, 0) as metric_pattern_weekday_5_count,
  pattern.weekday_5_last_visit_date as metric_pattern_weekday_5_last_visit_date,
  coalesce(pattern.weekday_6_count, 0) as metric_pattern_weekday_6_count,
  pattern.weekday_6_last_visit_date as metric_pattern_weekday_6_last_visit_date
