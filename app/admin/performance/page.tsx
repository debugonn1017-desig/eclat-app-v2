...
export default function PerformancePage() {
  ...
  // ─── 曜日別の来店件数 / 売上 ─────────────────────────────
  const [weekdayStats, setWeekdayStats] = useState<{ count: number[]; sales: number[] }>({
    count: Array(7).fill(0), sales: Array(7).fill(0),
  })
  const [weekdayMode, setWeekdayMode] = useState<'count' | 'sales'>('count')
  useEffect(() => {
    ...
    const fetchWeekday = async () => {
      ...
      if (data) {
        for (const v of data as any[]) {
          ...
          // 0円来店（場内チェック）は売上にも件数にもカウントしない方針
          //   → "来店パターン" を見たいので 0円は除外
          if (amt > 0) {
            cnt[dow] += 1
            sls[dow] += amt
      ...
      setWeekdayStats({ count: cnt, sales: sls })
    }
    fetchWeekday()
  }, [month, authorized, supabase])
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg }}>
      ...
      {/* ─── 曜日別パターン（ヒートマップ風バー） ─── */}
      {(() => {
        const arr = weekdayMode === 'count' ? weekdayStats.count : weekdayStats.sales
        const maxV = Math.max(...arr, 1)
        const labels = ['日', '月', '火', '水', '木', '金', '土']
        const colors = ['#D45060', '#5A2840', '#5A2840', '#5A2840', '#5A2840', '#5A2840', '#5080C0']
        const formatVal = (v: number) =>
          weekdayMode === 'count' ? `${v}件` : `¥${(v / 10000).toFixed(0)}万`
        return (
          <div style={{ padding: '0 20px 14px' }}>
            <div style={{
              ...
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: 11, color: C.pinkMuted, letterSpacing: '0.15em' }}>
                    曜日別の来店パターン
                  </div>
                ...
                <div style={{ display: 'flex', gap: 4 }}>
                  {([
                    ...
                    <button
                      key={opt.k}
                      onClick={() => setWeekdayMode(opt.k)}
                      style={{
                        padding: '5px 12px', fontSize: 11, borderRadius: 20,
                        background: weekdayMode === opt.k ? '#FBEAF0' : '#FFF',
                        color: weekdayMode === opt.k ? '#72243E' : C.pinkMuted,
                        border: `1px solid ${weekdayMode === opt.k ? '#ED93B1' : C.border}`,
