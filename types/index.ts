...
export type StaffPermission =
  ...
  | '売上入力'
  | 'シフト管理'
  // 細分化された下位権限（v2 - 2026-05-08 朝）
  | 'キャスト閲覧'    // 編集はせず閲覧のみ
  | 'お知らせ閲覧'    // 投稿はせず閲覧のみ
  | 'お知らせ投稿'    // 投稿のみ
  | 'レポート出力'    // PDF/CSV エクスポート
  // 細分化された下位権限（v3 - 2026-05-08 朝）
  | 'シフト閲覧'      // シフト一覧を見るだけ、編集不可
  | '売上閲覧'        // 過去日次売上を見るだけ、入力不可
  | '顧客閲覧'        // お客様詳細を見るだけ、編集不可
  // 高度な分析権限（v4 - 2026-05-08 夕方）
  | 'キャスト分析';   // /admin/casts/[id] の詳細分析ページを閲覧
...
export const PERMISSION_INCLUDES: Record<string, string[]> = {
  ...
  '売上入力': ['売上閲覧'],
  '顧客編集': ['顧客閲覧'],
  // 'レポート閲覧' と 'レポート出力' は包含なし（独立した別軸）
};

// ─── ロールプリセット ───────────────────────────────────────────
//   スタッフ追加時に「ホールスタッフ」「マネージャー」のように
//   よくある組み合わせを一括 ON できるプリセット。
//   付与時は既存の権限を保持したまま、プリセット内の権限だけ ON する（追加ベース）。
export type RolePresetKey = 'observer' | 'hall' | 'chief' | 'manager'

export const ROLE_PRESETS: Record<
  RolePresetKey,
  { label: string; description: string; permissions: StaffPermission[] }
> = {
  observer: {
    label: '観察役',
    description: '全画面を見るだけ。何も編集できない。新人ヘルプ・見学者向け',
    permissions: [
      '顧客閲覧',
      'キャスト閲覧',
      'お知らせ閲覧',
  ...
  hall: {
    label: 'ホールスタッフ',
    description: '観察役 + 売上入力 + お知らせ投稿。通常の現場スタッフ',
    permissions: [
      '顧客閲覧',
      'キャスト閲覧',
      'お知らせ閲覧',
  ...
  chief: {
    label: 'チーフ',
    description: 'ホール + シフト管理 + 顧客引継ぎ。現場リーダー',
    permissions: [
      '顧客閲覧',
      'キャスト閲覧',
      'お知らせ閲覧',
  ...
  manager: {
    label: 'マネージャー',
    description: 'ほぼ全権 + キャスト分析。オーナー以外の管理者向け',
    permissions: [
      '顧客編集',
      '顧客閲覧',
      'キャスト管理',
