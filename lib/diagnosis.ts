...
export function diagnoseCustomer(customer: Customer): DiagnosisResult {
  ...
  if (route === '前店舗顧客') {
    strat += '【既存の信頼】慣れによる雑さ厳禁。丁寧さを維持し「特別感」を継続させること。';
  } else if (route === 'SNS指名') {
    strat += '【高い期待値】SNSのイメージを崩さず、リアルでの接客密度で期待を上回れ。';
  } else if (route === '紹介指名') {
    strat += '【紹介者の顔】最初から信頼がある分、失礼は絶対NG。紹介者への報告も意識。';
  } else if (route === '店舗外指名') {
    strat += '【自然な出会い】営業感を出しすぎると冷める。偶然性と雰囲気を大切に。';
  } else if (route === 'J→本指名') {
    strat += '【接客の延長】場内からの流れを活かし、自然な形で関係を深める営業を。';
  } else if (route === 'H→本指名') {
    strat += '【比較の末】他キャストとの差別化が必須。「君じゃないとダメ」と思わせる。';
  } else if (route === 'R→本指名') {
    strat += '【最重要顧客】圧倒的な優先順位と特別感。ミス一つの損失が甚大。';
  } else if (route === 'F→本指名') {
    strat += '【短期決戦】関係が浅いため、2回目来店までのフォローを最優先に。';
  }
  ...
  else if (customer.phase === '安定' || customer.phase === 'リピート') strat += ' 他の客とは違う「唯一無二」の存在であることを強調。';
  ...
  } else if (route === 'R→本指名' && trend === '下降') {
    result.dangerAlert = 'ロイヤル客の離脱危機。最優先で個別の不満解消に動くべき。';
  } else {
  ...
  // 5. NG行動
  let ng = '';
  if (customer.ng_type === '既婚配慮必要') ng = '家庭の詮索、休日や深夜の不用意な連絡。';
  else if (customer.ng_type === '重い営業NG') ng = '露骨な売上協力要請、他客との比較。';
  else if (customer.ng_type === '連絡遅い') ng = '返信の催促、スタンプ連打。';
  else if (customer.ng_type !== 'なし' && customer.ng_type !== 'その他') {
    ng = `「${customer.ng_type}」に特に注意し、相手の不快感を避ける立ち回りを。`;
  } else {
    ng = isMarried ? '日曜連絡・夜21時以降の通知・家庭の詮索。' : '既読スルーへの追撃・感情的な発言・他店キャストの自慢。';
  }
  result.ngAction = ng;
  ...
  const generateMessages = () => {
    ...
    const cast = customer.cast_type || '清楚系';
    
    // キャストタイプ別の語尾・絵文字
    const styleMap: Record<string, { suffix: string, mid: string, aggressive: string, tone: string }> = {
      '清楚系': { suffix: '✨', mid: '😊', aggressive: '🎀', tone: 'お疲れ様です！' },
      '可愛い系': { suffix: '💕', mid: '✨', aggressive: '🍭', tone: 'お疲れ様！' },
      '綺麗系': { suffix: '🍷', mid: '✨', aggressive: '👠', tone: 'お疲れ様。' },
      '癒し系': { suffix: '🍀', mid: '🍵', aggressive: '🌸', tone: 'お疲れ様です。' },
      'お姉さん系': { suffix: '✨', mid: '💋', aggressive: '🥂', tone: 'お疲れ様、' },
      '色っぽい系': { suffix: '💋', mid: '🌹', aggressive: '💄', tone: 'お疲れ様…💕' },
      'ノリ良い系': { suffix: '🔥', mid: '🙌', aggressive: '⚡️', tone: 'お疲れー！' },
      'ギャル系': { suffix: '💖', mid: '✌️', aggressive: '💅', tone: 'おつー！' },
      'その他': { suffix: '✨', mid: '😊', aggressive: '🌟', tone: 'お疲れ様です！' }
    };

    const style = styleMap[cast] || styleMap['その他'];
    ...
    } else if (customer.spouse_status === '有') {
