...
- 「今日の確認」は「今日までの追いかけ」「基本情報の不足」の2項目に整理
- 頻度の高い操作として「新しいお客様を登録」ボタンを追加し、既存の `/new` へ直接遷移
- 登録ボタンはキャスト、オーナー、または `顧客.編集` 権限のある管理スタッフだけに表示
- キャストが「基本情報の不足」を押した場合は `/customers?incomplete=incomplete` へ遷移し、
  お客様一覧で「未登録あり」を最初から適用して検索する。通常の `/customers` は従来どおり
