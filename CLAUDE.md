...
- 左のキャスト一覧には、キャストページと同じ「現在本指名かつ福岡県」の県内顧客人数を常時表示する。人数を押すと、そのキャストページの顧客タブを既存オーバーレイで直接開く。
- 県内顧客人数は全顧客の重い集計ビューを使わず、必要な条件・2列だけを管理者APIで一括取得する。選択中キャストの詳細集計と既存3一覧の定義は変更しない。
