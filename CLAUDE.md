...
## v0.3.64: 追いかけ中の地域フィルター（2026-07-28）

- 追いかけ中の「本指名 / 場内 / フリー / 未設定・その他」の各分類内に、
  `すべて / 福岡県 / 県外 / 地域未設定` の人数付きフィルターを追加
- 地域判定は、trim後が「福岡県」なら福岡県、入力済みのその他の値は県外、
  NULL・空文字・空白のみは地域未設定とする
- 指名分類を切り替えた際は地域を「すべて」へ戻し、前の条件による見落としを防ぐ
- 表示上の絞り込みだけを追加し、DB・API・RLS・通知・期限・顧客分類・KPIは変更しない

## v0.3.65: 追いかけカードの閲覧・編集分離（2026-07-28）

- 追いかけ中のお客様カードは通常時に入力欄を表示せず、確認に必要な情報へ集約
  - 一番近い期限を「期限超過 / 今日まで / あと○日 / 未設定」で色分け
  - 次にやること、営業連絡期限、再来店期限、メモ、最終連絡を一覧で確認できる
  - 「連絡した」は通常表示からすぐ実行できる
- 各カード右上の「編集」を押したカードだけ、行動・期限・メモの編集フォームを表示
  - 保存成功時だけ通常表示へ戻る
  - 保存失敗時は編集内容を残し、修正して再実行できる
  - キャンセル時は保存せず通常表示へ戻り、次回編集時にDBの最新値から開始する
- PCでは期限・行動・メモを横並び、スマホでは縦方向へ切り替えるレスポンシブ表示
- DB・API・RLS・期限計算・通知条件・並び順・追いかけの追加/Undoには変更なし
- hotfix: 営業連絡・再来店期限と内容が重複していた「一番近い期限」パネルを削除
- 名前を大きくし、ランク・指名状況・地域・担当キャストを色分けしたバッジで表示
- メモの文字サイズと表示行数を増やし、最終連絡と視覚的に分離
- 指名状況の絞り込み先頭に「全て」を追加し、初期表示も全員へ変更

## v0.3.66: キャストKPIの表示整理（2026-07-28）

- 成績画面で縦に並んでいた次の5項目を「詳しい成績」タブへ集約
  - ノルマ達成状況
  - ランク別売上
  - ランク別来店回数
  - 同伴・アフター実績
  - 場内→本指名 転換トラッキング
- 実出勤日数・出勤予定日・設定出勤日数は、通常の小カードから独立した
  常時表示の強調カードへ変更
- 上部タブの「予定」を「出勤設定」へ変更し、用途を分かりやすくした
- 「おすすめ客像」は上部タブの一番右、ランキングの右側へ移動
- 上部タブの表示順とスマホ左右スワイプ順は同じ共通配列を使い、内部KPIタブの
  横スクロール中は上部タブが切り替わらないよう競合を防止
- KPIの集計値・ノルマ定義・出勤日数の計算・転換履歴・API・DBには変更なし

## v0.3.67: 顧客ごとの来店曜日・時間帯傾向と並び替え（2026-07-30）

### オーナー確定ルール

- キャスト詳細の顧客カードと顧客検索カードに、直近10回の実来店から
  来店曜日（上位2曜日）と時間帯を表示する
- 時間帯は `20時台 / 21時台 / 22時台 / 23時台 / 0時台` の5区分
  - 20→21→22→23→0時台の順で、早い時間の実績を優先表示・優先ソートする
  - 20時台の実績が1回でもあれば、23時台の来店が多くても20時台を先に表示する
  - 3回以上は「来店傾向」、1〜2回は断定を避け「来店実績」と表示する
- 予定来店は除外し、0円の実来店は来店傾向へ含める
- 0時台の曜日は、既存の店舗営業日 `visit_date` をそのまま使う

### 並び替え

- キャスト詳細は「顧客 / 県外顧客 / 地域未設定 / ランクC / その他 /
  場内 / フリー / 切れた」のカテゴリと人数を一切変えず、カテゴリ内だけを並び替える
- 顧客検索は全検索結果をDB側で並び替えた後に50件ずつページングする
- 選択肢は、標準またはお客様名 / 早い時間の実績 / 最終来店が古い・新しい /
  累計売上 / 来店回数 / 客単価（検索は既存のランク・最終連絡・指名状況も維持）

### DB・権限・品質

- `customer_visit_patterns` を SECURITY INVOKER view として追加し、
  `customer_search_metrics` の末尾に傾向列を追加する
- 既存 customers / customer_visits RLSを維持し、キャストは担当顧客だけを閲覧できる
- KPI・顧客分類・自動ランク判定・追いかけ・売上集計の定義は変更しない
- JS集計の仕様固定テストをCIへ追加。本番反映順序は migration → verification → deploy

## v0.3.68: 月〜土の曜日別来店実績順（2026-07-30）

- キャスト詳細の顧客リストと顧客検索に、次の並び替えを追加する
  - 月曜日の来店実績順
  - 火曜日の来店実績順
  - 水曜日の来店実績順
  - 木曜日の来店実績順
  - 金曜日の来店実績順
  - 土曜日の来店実績順
- 選択した曜日について、直近10回の実来店内の「回数が多い順」→
  「その曜日の最終来店が新しい順」で並べ、実績なしは下に表示する
- 同じお客様に複数曜日の実績があってもカードは1枚のまま。
  選択中の曜日の実績だけを順位に使用する
- キャスト詳細は既存カテゴリと人数を変えず、各カテゴリ内だけを並び替える
- 顧客検索はDB全件ソート後に50件ずつページングする
- v0.3.67と同じく予定来店を除外し、0円の実来店と1〜2回の実績も対象にする
- `customer_visit_patterns` / `customer_search_metrics` は既存列順を維持し、
  月〜土の回数・最新日を末尾へ追加。SECURITY INVOKER と既存RLSを維持する

## v0.3.69: 顧客カードのコンパクト化（2026-07-30）

- キャスト詳細の顧客カードを、全情報を残したまま一覧性の高いレスポンシブ表示へ変更
  - スマホは名前・属性・来店売上・曜日時間帯・担当/追いかけの5つの薄い情報帯に整理し、
    従来より縦幅を約25〜30%削減
  - PCは1人1行の横長レイアウトにし、同じ情報を横方向へ整理
  - カテゴリ分類・カテゴリ人数・カテゴリ内の並び替え・スワイプ・複数選択は従来どおり
- 曜日・時間帯は、月〜土の実績をドット、20/21/22/23/0時台を小さなチップで表示
  - 選択中の曜日並び替えでは、対象曜日の回数と最終日を強調
  - 1〜2回は「来店実績」、3回以上は「来店傾向」、実績なしも空状態を表示
- 顧客検索カードにも同じ曜日・時間帯の小型サマリーを適用し、見え方を統一
- 実来店0件のカードは、見出しも「実績なし」に統一
- API・DB・RLS・来店傾向の集計定義・顧客分類・KPIには変更なし

## v0.3.70: キャスト詳細の顧客カードを3列に整理（2026-07-30）

- キャスト詳細の顧客一覧を、PC・スマホとも次の3列で読めるカードへ変更
  - 左: お客様名・ニックネーム・ランク・指名状況・年代・地域
  - 中央: 累計売上・客単価
  - 右: 来店回数・最終来店・早い時間帯
- 曜日別・早い時間・最終来店・売上・客単価の並び替え中は、該当する値を
  カード内で強調し、「なぜこの順番なのか」を一覧だけで確認できるようにした
- カード左端のラインは、ランクや期限を表す色分けを廃止してエクラピンクへ統一
- 同伴者と追いかけの詳細は一覧から外し、必要なときに顧客詳細・追いかけ画面で確認する
  - 「追いかけ中」バッジは残し、PCではバッジにカーソルを合わせると次の行動を確認できる
- 一覧で使わなくなった同伴者取得クエリを削除し、CUSTOMERSタブを開く際の通信を1回削減
- 顧客カテゴリ・カテゴリ人数・カテゴリ内の並び替え・スワイプ・複数選択・
  追いかけ追加・切れた移動・顧客詳細・API・DB・RLS・KPIの定義は変更しない

## v0.3.71: PC顧客カード復元・スマホ補強・個人ページ来店傾向（2026-07-30）

- キャスト詳細CUSTOMERSタブは端末別に表示を分ける
  - PC: v0.3.69の横長1行カード（基本情報 / 来店・売上 / 曜日・時間帯 /
    担当・お連れ様・追いかけ）へ戻す
  - スマホ: v0.3.70の3分割カードを維持する
- ランクはS/A/B/C/切れた/未設定ごとの色付き強調バッジにし、PCには円形ランク表示も復元
- スマホの来店列に、直近10回で多い上位2曜日を追加
- スマホの基本情報列に、最新来店で登録されたお連れ様の本指名・場内指名を1行で表示
  - 長いキャスト名はカード内で省略し、詳細はtitle属性と顧客詳細で確認できる
- 顧客個人ページ・顧客詳細オーバーレイに「来店しやすい曜日・時間帯」を追加
  - 既に取得済みの来店履歴から `buildCustomerVisitPatterns` を使って算出するため、
    個人ページでは追加通信を行わない
  - 予定来店を除外し、直近10回・20→21→22→23→0時優先などv0.3.67の定義を再利用
- お連れ様取得はCUSTOMERSタブを開いた時だけ1回実行し、RLSの可視範囲を維持
- API・DB・migration・顧客分類・カテゴリ人数・並び替え・KPI・自動ランク判定は変更しない

### v0.3.71 hotfix: スマホ顧客カードの3列配置修正

- PC用の `grid-area: identity` がスマホ用3列グリッドにも残り、名前欄が暗黙領域へ
  移動して大きな空白が生じるCSS競合を修正
- スマホカードは、基本情報=1列目・売上=2列目・来店=3列目を明示して配置する
- PC表示、カード内の情報、顧客分類・人数・並び替え・操作・業務ロジックは変更しない

## v0.3.72: 追いかけリストの並び替え（2026-08-05）

- 追いかけ中タブに並び替えを追加する
  - 対応優先順（従来の並び・初期値）
  - 追加が新しい順 / 古い順
  - 最終連絡が古い順（未連絡優先）/ 新しい順
  - 再来店期限が近い順 / 営業連絡期限が近い順 / お客様名順
- 追加日は `created_at` ではなく、現在の追いかけ開始日 `activated_at` を使用する。
  一度外して再追加した場合は再追加日が基準になる
- 本指名・場内・フリー・その他と地域の絞り込み後の一覧内だけを並び替える
- API・DB・RLS・通知・期限計算・候補・履歴・追いかけ人数は変更しない
- 並び替えロジックは `lib/followUpWorkflow.ts` に集約し、仕様固定テストを追加する

## v0.3.73: キャスト詳細の顧客検索（2026-08-06）

- キャスト詳細のCUSTOMERSタブに、普段は閉じている小型の検索ボタンを追加する
- 検索欄を開いた場合だけ、お客様名・ニックネームの部分一致で画面内検索する
  - Unicode NFKC正規化と空白除去により、全角/半角や姓名間の空白差を吸収する
  - 検索結果の人数を表示し、0人の場合は専用の空表示を出す
- 顧客/県外顧客/地域未設定/ランクC/その他/場内/フリー/切れたの分類は維持し、
  一致したお客様を元のカテゴリ内に表示する
- 検索中だけ一致カテゴリを自動展開し、検索を閉じると検索前のカテゴリ開閉状態へ戻る
- カテゴリ内の並び替え、スワイプ、複数選択、追いかけ追加、切れた移動、Undoは変更しない
- 既に取得済みの担当顧客をクライアント側で絞り込むため、API・DB・RLS・通信回数に変更なし

## v0.3.74: スワイプから追いかけ追加・解除（2026-08-06）

- 顧客カードを左スワイプしたときの追いかけボタンを、現在状態に応じて切り替える
  - 追いかけ前: `追いかけ`
  - 追いかけ中: `追いかけ解除`
- 対象は、追いかけ操作のある全画面
  - キャスト詳細CUSTOMERSタブ
  - お客様検索
  - 接客カレンダーの日別一覧
- `切れたへ` は従来どおり右側に残し、ランク変更・Undo・追いかけ保持仕様を変更しない
- 解除直前に追いかけ状態を再取得し、最新行のidを使って `action: remove` を実行する
  - 最新状態を取得できない場合は、古い画面状態で解除せずエラー表示する
- 解除後は8秒間の「元に戻す」を表示し、`action: reactivate` で復旧する
- 期限・次の行動・メモ・連絡履歴は削除せず、既存の追いかけ行を非アクティブ化する
- API・DB・migration・RLS・権限・一括追加・切れた操作は変更しない

## v0.3.75: ボトル名検索 + 最終来店経過日数の視認性改善（2026-08-06）

- お客様を文字検索できる4画面で、ボトル名も検索対象に追加
  - お客様一覧
  - キャスト詳細CUSTOMERSタブ
  - 情報不足チェック
  - 日次売上入力の顧客選択
- サーバー検索用に SECURITY INVOKER のラッパービューを2本追加
  - `customer_search_metrics_with_bottles`
  - `customer_core_quality_with_bottles`
  - 既存ビューは置換せず、既存列・件数・並び替え・RLSを維持する
- キャスト詳細の検索は、既存customer-meta APIで担当顧客のボトル名を一括取得
  - 200顧客単位のチャンク + ページングで、顧客ごとのN+1は行わない
  - 同APIを使えるKPI閲覧のみのスタッフにはボトル名を返さず、キャスト本人・
    オーナー・顧客閲覧権限ありに限定する
- 日次売上入力も選択中キャストのボトル名だけを一括取得し、検索候補へ反映
- キャスト顧客カードの最終来店経過日数を改善
  - PCは色付きピルを拡大
  - スマホは「○日前」を独立した太字・枠付きピルで表示
  - カードの縦幅・カテゴリ・並び順・経過日数の計算定義は変更しない
- 適用順序は migration → verification SQL → deploy

## v0.3.76: 接客カレンダー日別一覧の絞り込み・並び替え（2026-08-06）

- 日付を押して開く接客一覧に、普段は閉じている小型の「絞り込み」ボタンを追加
- 日別一覧内で次の条件を組み合わせられる
  - 指名状況: すべて / 本指名 / 場内 / フリー
  - 内容: すべて / 同伴あり / アフターあり / 追いかけ中 / 売上あり
  - 担当キャスト: 全キャスト / キャスト別（管理者が全体表示中の場合のみ）
  - 並び替え: 記録順（従来）/ 来店時間が早い順 / 遅い順 / 売上が高い順
- 絞り込み後の表示件数・売上をパネル内に表示し、0件時は専用文言を表示する
- 来店時刻を日別カードに追加し、時間順の結果を一覧上でも確認できるようにする
- 日を開き直すと条件は初期値へ戻り、初期表示の件数・売上・並び順は従来どおり
- 条件変更時は一括選択を解除し、画面から隠れたお客様への誤操作を防ぐ
- 「追いかけ中」の状態取得は編集権限とは分離し、閲覧専用スタッフもAPIの
  顧客閲覧チェックとRLSの範囲内で絞り込めるようにする
- API・DB・migration・RLS・追いかけ・切れた・Undoの仕様は変更しない

## v0.3.77: スマホ版お客様カードの視認性改善（2026-08-06）

- キャスト詳細CUSTOMERSタブのスマホカードを、名前が横一列で読める構成へ変更
  - 上段: お客様名・ニックネーム・操作ボタン
  - 中段: 新規/追いかけ・ランク・指名状況・年代・地域・お連れ様
  - 下段: 売上/客単価と、最終来店日/経過日数/来店傾向の2列
- 最終来店は日付を大きく表示し、経過日数を従来の色分けピルで併記する
- 曜日順・早い時間順では、選択中の並び替え実績を来店欄に残す
- PCカード、分類・人数・並び替え、スワイプ、複数選択、追いかけ、切れた、Undoは変更しない
- API・DB・migration・RLSへの変更なし
## v0.3.78 — 追いかけ優先度（2026-08-11）

- 追いかけ中のお客様へ手動の優先度「最優先／高／中／低」を追加した。既存行・新規行の初期値は「中」。
- 通常カードに色付きの優先度ラベルを常時表示し、編集画面から変更できる。
- 指名状況・地域の既存分類を維持したまま、優先度の絞り込みと「優先度が高い順」の並び替えを追加した。同一優先度では対応期限が近い順とする。
- 従来の「対応優先順」は意味を明確にするため「対応期限が近い順」へ表示名だけ変更した。期限・14時通知・ランク判定・追いかけ解除には優先度を使用しない。

## v0.3.79 — 担当者チェック・追いかけログ（2026-08-11）

- 追いかけカードの「連絡した」ボタンを廃止し、「担当者チェック」と「ログ確認」へ置換。
- 担当者チェックは、未読無視／既読無視／返信あり／仮来店／来店予定の1件を選び、任意メモとログインアカウントの監査情報を追記保存する。保存直後は8秒間取り消せる。
- お客様詳細タブを「基本情報／接客方針／LINE／追いかけログ／来店履歴／ボトル」の順にし、追いかけ開始・終了、LINE送受信、担当者チェック、実来店の日時・時間帯・金額を1本のタイムラインで確認できるようにした。
- 実来店は予定来店を除外し、現在の追いかけ期間開始後だけ反映する。来店後も追いかけ中・優先度・次の行動・メモは保持し、営業連絡目安と再来店期限の基準日だけを実来店日に更新する。
- 営業連絡間隔は強制期限ではなく「目安」として表示する。担当者チェック・実来店・間隔設定変更を基準に数え、メモだけの保存では先送りしない。
- 追いかけ内容の未設定は「次の行動が空、またはメモが空」と定義し、未設定／設定済みで絞り込める。カードには不足理由を表示する。
- PC管理者画面は左側に層別キャスト一覧と追いかけ中人数を表示する。キャスト本人画面とスマホでは従来の選択導線を維持する。
- DBは `follow_up_activity_logs` を追加し、開始・終了ログを親行更新と同一トランザクションのトリガーで保存する。内部SECURITY DEFINER関数はauthenticatedから直接実行不可とする。
- 適用順序は migration → verification SQL → deploy。レビューOK前は本番適用しない。
- hotfix: `cast_tier: null` と入店日の同時送信でも新人層限定を回避できないよう、APIの実効キャスト層判定でnullを明示値として扱う。

## v0.3.80 — スマホ版追いかけカードの圧縮表示（2026-08-11）

- PC版の追いかけカードは変更せず、700px以下のスマホ版だけ閲覧表示を圧縮する。
- 追いかけ中一覧のスマホ表示を「対応／実績／メモ」の3タブに分け、全カードを同じ観点で一覧比較できるようにする。
  - 対応: 次の行動を全件、営業連絡目安、再来店期限、最新チェック、担当者チェック
  - 実績: 累計売上、来店回数、最終来店、追いかけ開始後の来店
  - メモ: 追いかけメモと最新チェック
- 名前・ニックネーム・ランク・指名状況・地域・担当キャスト・優先度・編集導線は、どのタブでもカード上段に残す。
- 既存の指名状況・地域・優先度・内容・並び替えは、スマホでは1行の開閉パネルへ畳み、設定中の条件数を表示する。PCでは従来どおり常時表示する。
- 「詳細を開く」でメモ・実績・ログ・お客様詳細・リストから外す操作を展開し、普段の一覧は1画面で約3人を確認しやすい高さにする。
- 編集画面、候補・履歴、絞り込み、並び替え、担当者チェック、ログ、期限計算、通知、API・DB・RLSは変更しない。
- hotfix: スマホの「次にやること」は先頭2件＋残数への省略をやめ、設定済みの全項目をコンパクトなタグで折り返して表示する。

## v0.3.81 — 新人層の90日育成ナビ（2026-08-22）

- 対象は現在の `cast_tier = '新人層'` のキャストだけ。他層のキャストにはタブ・ホーム案内を表示しない。
- `profiles.training_start_date` に実際の入店日を保存し、JSTの今日との差から現在STEPを画面表示時に自動計算する。アカウント作成日は使用しない。
- 自動STEPは 1〜4日=STEP1 基礎接客、5〜9日=STEP2 ドリンク営業、10〜14日=STEP3 連絡先交換、15〜28日=STEP4 場内・延長、29〜42日=STEP5 アフター、43〜56日=STEP6 営業連絡、57〜90日=STEP7 リピート・再来店計画。
- 91日目以降は「90日育成期間終了・確認待ち」と表示するだけで、キャスト層を自動変更しない。KPI・ランク判定・顧客分類にも使用しない。
- キャスト詳細に「90日育成」タブを追加し、現在STEP・経過日数・進捗・次STEPまでの日数と7STEPのロードマップを表示する。
- 各STEPの「マニュアルを見る」は既存 `/manual/data.json` と既存マニュアルコンポーネントをオーバーレイ表示し、教材を二重管理しない。
- 入店日の更新は既存 `キャスト.アカウント管理` 権限を持つ黒服・オーナーだけ。APIでも日付形式と新人層を検証し、非新人層への新規設定を拒否する。キャスト本人は自分の現在地とマニュアルを閲覧する。
- 新人キャスト本人のホームには現在STEPへの短い導線を表示し、PCキャスト一覧には新人のSTEPバッジを表示する。
- 適用順序は migration → verification SQL → deploy。レビューOK前は本番適用しない。

## v0.3.82 — キャスト一覧に新人育成の現在地を表示（2026-08-22）

- `/casts` の新人層キャスト行に、入店日から自動計算した「入店○日目」と
  「STEP○ 工程名」を表示する。
- 入店日前は「入店まであと○日／開始前」、91日目以降は
  「入店○日目／90日育成完了」、入店日未設定時は「入店日未設定」と表示する。
- STEP計算は v0.3.81 の共通関数を再利用し、KPI・キャスト層・育成データは変更しない。
- API・DB・migration・RLSへの変更なし。

## v0.3.83 — 黒服専用MTログ（2026-08-22）

- キャスト詳細に黒服専用の「MTログ」タブを追加する。キャスト本人のタブ一覧には表示せず、URL直指定も認証確定後に成績タブへ戻す。
- MT日・題名・担当者・AIボイスレコーダーの文字起こし全文を追記保存し、新しい順の縦タイムラインで表示する。
- ログを押すと全文をPCでは中央オーバーレイ、スマホでは全画面オーバーレイで確認できる。Escape・背景クリック・閉じるボタンに対応する。
- 入力した担当者とは別に、登録操作を行ったログインアカウントIDと表示名を監査情報として自動保存する。
- 記録の改ざん・誤削除を避けるため、v0.3.83では追記・閲覧だけとし、編集・削除は提供しない。
- UIだけでなくAPIで `role = admin` を必須とし、DB RLSもadminのSELECT/INSERTだけを許可する。キャスト本人はAPI直呼び・Supabase直アクセスでも閲覧できない。
- 適用順序は migration → verification SQL → deploy。独立レビューOK前は本番適用しない。

## v0.3.84 — 顧客集計の表示速度改善（2026-08-27）

- 機能・件数・分類・並び順・権限を変えず、重い顧客集計ビューの実行経路だけを改善する。
- 調査時点の本番データは顧客6,841人・来店9,003件。`pg_stat_statements` では
  `customer_search_metrics_with_bottles` が平均約1.8秒、重い実行で約3〜7秒だった。
- MTログは26件・文字起こし全文合計約6.3万文字で、MTタブを開いたときだけ動的読込される。
  アプリ全体の初期表示が遅い主因ではないため、MTログの機能・取得方法は変更しない。
- お客様検索・情報不足チェック・追いかけ一覧では、先に通常セッションで本人確認・
  権限確認・担当範囲を確定し、来店集計を含む重いビューだけをサーバーのservice roleで読む。
  - キャストは必ず本人の `cast_name` に限定する。
  - URLで他キャスト名を指定した場合も、従来のRLSと同じ積集合となり0件を維持する。
  - 管理者は従来どおり顧客閲覧権限を確認し、指定キャストがあればその担当だけに限定する。
  - 追いかけ一覧は、通常セッションとRLSで取得済みのcustomer idだけを集計対象にする。
- 担当範囲の決定を `lib/customerQueryScope.ts` に分離し、管理者・キャスト・URL改変・
  担当名未設定の6ケースを自動テストで固定する。
- キャスト詳細のサイドバー用profiles取得は `select('*')` をやめ、表示に必要な8列だけにする。
- 新規migration・DB定義変更・RLS変更はなく、デプロイだけで反映できる。
- 検証基準: typecheck 0、全テスト80件成功、対象lint 0。全体lint 109件
  （48 errors / 61 warnings）は変更前からの既存基準で増減なし。

## v0.3.85 — 顧客検索で退店キャストを指定（2026-08-27）

- 黒服・オーナーの顧客検索に、担当キャストを「在籍キャスト／退店キャスト」で分けた選択肢を追加する。
- 退店キャストを選ぶと、現在の `customers.cast_name` がそのキャスト名のお客様を従来の検索APIで絞り込む。
- 顧客検索専用APIは、顧客閲覧権限を確認した管理者にだけ、キャスト名・表示名・在籍状態の最小項目を返す。
- キャスト本人には退店キャスト名簿を返さず、検索APIも従来どおり本人の `cast_name` に固定する。
- 顧客の担当が別キャストへ変更済みの場合は現在担当で検索され、過去担当履歴としては扱わない。
- DB・migration・RLS・顧客分類・KPI・ランク判定への変更なし。

## v0.3.86 — キャスト一覧に退店キャストを追加（2026-08-28）

- `/casts` に黒服・オーナーだけが見られる「在籍キャスト／退店キャスト」の切替を追加する。
- 在籍キャストは従来の層別一覧をそのまま維持し、退店キャストは層へ混ぜず独立一覧にする。
- 退店一覧はタブを開いたときだけ遅延取得し、在籍一覧の初期表示速度を変えない。
- 退店キャストの行には元の層と「退店」を表示し、押すと既存のキャスト詳細を開く。詳細ヘッダーにも「退店」を表示する。
- 退店一覧APIは `requireAdmin()` とprofiles RLSの両方で保護し、キャスト本人のAPI直呼びは403とする。
- DB・migration・RLS・KPI・顧客分類・ランク判定・MTログへの変更なし。

## v0.3.87 — お客様担当黒服の個人ページ（2026-08-28）

- 黒服権限に「顧客.担当」を追加。この権限は顧客閲覧・編集を含み、お客様の新規登録も行える。
- 顧客登録・編集の「お客様担当あり」を、権限が付いた黒服アカウント名の複数チェックに置換。1人の顧客に複数の黒服を割り当てられる。
- 旧 `has_customer_staff=true` だけのデータは「担当者名未設定」として保持し、別項目の編集だけで解除しない。
- 担当者の入れ替えと `has_customer_staff` の同期は service_role 専用RPCの1トランザクションで確定し、一部だけ保存される事故を防ぐ。
- `/casts` に黒服・オーナー専用の「お客様担当」一覧を追加。個人ページは「担当顧客」と「売上・来店」の2タブだけとする。キャスト本人はUI・APIの両方で閲覧不可。
- 各顧客の担当キャストと店舗KPI・顧客分類は変更しない。お客様担当の売上・来店は割当顧客の閲覧用集計で、複数担当でも店舗合計を二重加算しない。
- キャストの顧客カードと顧客詳細に、割り当てられたお客様担当のアカウント名を表示する。
- 適用順序は migration → verification SQL（true/0を確認）→ deploy。独立レビュ完了前は本番反映しない。

## v0.3.88 — お客様担当ページのランク別顧客一覧（2026-08-28）

- お客様担当個人ページの「担当顧客」を、キャストページに近いコンパクトな顧客カード表示へ変更する。
- 担当顧客を `S / A / B / C / 切れた / 未設定` の6グループへ排他的に分類し、グループごとの人数を表示する。担当割当・店舗KPI・キャスト側カテゴリ分類は変更しない。
- 名前・ニックネーム・ボトル名検索、お客様ランク・指名状況・地域のAND絞り込み、カテゴリ内の並び替えを追加する。
- 並び替えは各ランクグループの内部だけに適用し、「標準」でお客様担当へ追加された順へ戻す。
- PCは横長一覧、スマホは基本情報＋売上＋最終来店の2段カードにして、長い名前・金額でも横にはみ出さない構成とする。
- 「売上・来店」タブ、黒服・オーナー限定の認可、顧客詳細への導線は従来どおり維持する。
- APIは割当済み顧客IDに限定してボトル名を一括取得し、検索用文字列だけを返す。N+1・DB・migration・RLS変更なし。
- 検証基準: typecheck 0、全テスト91件成功（新規6件を含む）、対象lint 0。全体lint 109件（48 errors / 61 warnings）は変更前基準から増減なし。

## v0.3.89 — 課題見える化シート（2026-08-28）

- 黒服・オーナー専用の `/admin/cast-issues` を追加し、キャスト本人はUI・APIとも閲覧不可とする。
- PCでは左側に在籍キャストを層別表示し、選んだキャスト1人分だけを右側へ集計する。全キャストの来店履歴を一括取得せず、既存画面の初期表示を重くしない。
- 右側は上から「本指名・直近4週間」「本指名・周期遅れ」「場内・直近4週間」の固定順で表示し、3一覧をそれぞれ独立して開閉できる。
- 各一覧は「福岡県／県外／地域未設定」に分け、お客様を押すと既存 `CustomerDetailPanel` をオーバーレイ表示する。
- 直近4週間はJSTの今日を含む28日間。予定来店は除外し、0円の実来店は回数へ含める。
- 周期遅れは既存顧客詳細と同じ、全来店間の正の日数差を平均した来店周期を使用する。最終来店からの経過が平均周期を7日以上超えた本指名を対象にする。
- 場内獲得は `nomination_history` の `new_status='場内'` を使用し、同じ顧客に複数履歴がある場合は対象期間内の最新獲得日を表示する。その後に本指名へ転換済みでも獲得実績として残す。
- キャスト概要に4週間の来店人数・売上・周期遅れ・場内獲得に加え、既存ノルマ階層から解決した設定売上・設定出勤と今月の実出勤日数を表示する。
- DB・migration・RLS・KPI・顧客分類・ランク判定への変更なし。

## v0.3.90 — 課題見える化シートの顧客情報を充実（2026-08-28）

- 課題見える化シートのお客様行を、キャストページの顧客一覧に近い横長の情報カードへ変更する。
- 名前・ニックネーム・ランク・指名状況・年代・地域・最終来店からの経過日数・最終連絡日を基本情報として表示する。
- 累計来店回数・累計売上・累計客単価と、直近10回来店を使った曜日・時間帯の来店傾向を表示する。
- 最新のお連れ様指名情報、割り当て済みのお客様担当、追いかけ中の次の行動・再来店期限を一覧上で確認できるようにする。
- 既存の「本指名・直近4週間」「本指名・周期遅れ」「場内・直近4週間」には、それぞれの判断に必要な4週間実績・周期超過・場内獲得情報を残す。
- 予定来店の除外、0円実来店の回数算入、3一覧の定義・人数・順序、地域分類、詳細オーバーレイ、管理者限定の認可は変更しない。
- 追加データは選択中キャストの担当顧客IDだけをチャンク取得し、N+1を作らない。DB・migration・RLSへの変更なし。

## v0.3.91 — 課題見える化シートにボウズ状況とキャスト詳細を追加（2026-08-29）

- キャスト概要カードを大きくし、キャスト名を押すと既存キャストページをPCでは右ドロワー、スマホでは全画面オーバーレイで表示する。
- オーバーレイ内は既存キャストページを押したときだけ遅延読込し、サイドバーとBottomNavを重複表示しない埋め込み表示にする。
- ボウズは「出勤／来客出勤の日に、現在本指名の担当顧客の実来店が0件」と定義する。場内・フリーの来店や予定来店は本指名来店に含めない。
- 過去4週間の確定出勤日数とボウズ日数、直近の確定出勤日から続く連続ボウズ出勤数を表示する。休み・希望出勤は数えず、営業途中の当日は誤判定防止のため確定対象から除く。
- ボウズ計算を純粋関数に分離し、場内来店・予定来店・0円本指名来店・休み・当日除外・連続日数を自動テストで固定する。
- DB・migration・RLS・KPI・顧客分類・ランク判定への変更なし。

## v0.3.92 — 課題見える化シートの期間・実績・操作強化（2026-08-29）

- 対象期間を「現在から過去4週間」と「月ごとの月末時点」で切り替えられるようにする。未来月は選択不可とする。
- キャスト概要に、対象月の実売上・設定売上・差額・達成率、対象期間の本指名ユニーク人数・本指名本数・本指名売上を表示する。
- 本指名直近期間・周期遅れ・場内獲得の3一覧それぞれに、業務目的に合わせた並び替えを追加する。地域3分類と人数は並び替えで変えない。
- お客様カードは、左から「基本情報→対象期間の数字→来店傾向→累計数字→お連れ様・お客様担当・追いかけ」の順に統一する。
- お客様カードを左スワイプ、またはPCの「⋯」から、追いかけ追加／解除と「切れたへ」を実行できるようにする。既存の8秒Undoと越権防止APIを共用し、「切れたへ」で追いかけは解除しない。
- `customer_visits.nomination_status_at_visit` を追加し、既存履歴を指名履歴からJST来店時刻基準で補完する。以後はtriggerで来店時点の指名状況を保存し、現在の指名状況を変更しても過去月の本指名実績・ボウズが変わらないようにする。
- 過去月の周期遅れ・カード上の指名状況も、`nomination_history` から月末時点の状態を復元する。
- 適用順序は migration → verification SQL（true/0確認）→ deploy。新アプリは追加列を読むため順序を厳守する。

## v0.3.93 — 周期遅れから切れた顧客を除外（2026-08-29）

- 課題見える化シートの「本指名・周期遅れ」は、本指名かつお客様ランクが「切れた」ではない顧客だけを表示する。
- 切れた顧客の除外は一覧作成時に行い、周期遅れの人数・地域別件数・並び替え結果も同じ定義に揃える。
- 直近期間の本指名実績、場内獲得、ボウズ、売上、来店履歴、追いかけ操作は変更しない。
- API・DB・migration・RLSへの変更なし。

## v0.3.94 — 課題見える化シートの会議用UI（2026-08-29）

- 課題見える化シートを、濃紺・白を基調とする黒服／オーナー向けの会議用ダッシュボードへ刷新する。
- 既存の期間切替、集計値、3一覧、地域分類、並び替え、詳細オーバーレイ、追いかけ／切れた操作は変更しない。
- 各地域のお客様一覧に「お客様情報／対象期間の実績／来店傾向／累計実績／担当・追いかけ」の固定見出しを追加し、表示内容の位置関係を明確にする。
- 「会議モード」では通常ヘッダーと下部ナビを非表示にし、課題見える化シートを画面全体で表示する。Escape または「通常表示に戻る」で解除できる。
- 優先課題の自動抽出は、判定項目が未確定のため本バージョンには含めない。

## v0.3.95 — 課題見える化シートのお客様ナンバリング（2026-08-29）

- 「本指名・対象期間」「本指名・周期遅れ」「場内・対象期間」の各一覧で、お客様名の左に表示順の番号を追加する。
- 番号は各一覧ごとに1から振り直し、福岡県・県外・地域未設定をまたいで連番にする。
- 並び替えを変更した場合は、画面に表示される新しい順番に合わせて番号も振り直す。
- 集計、地域分類、並び替え条件、お客様詳細、追いかけ／切れた操作、API・DB・migration・RLSは変更しない。

## v0.3.96 — 課題見える化シートの優先課題（2026-08-29）

- 会議で最初に確認する「優先する課題」を4カードで追加し、カードを押すと判定対象のお客様名を表示する。お客様名を押すと既存のお客様詳細を開く。
- 数の追求は、対象期間に本指名として実来店した福岡県のお客様のユニーク人数を15人目標で判定する。予定来店は除外し、0円の実来店は1人として数える。
- 質の追求は、既存ノルマ階層から解決した設定売上を45で割った値を設定単価とし、県内本指名のお客様ごとの対象期間客単価が届いているか判定する。
- 月3回来店は、県内本指名のお客様のうち対象期間に3回以上実来店した人数を15人目標で判定する。
- 場内からの追いかけは、対象期間に場内を獲得したお客様のうち、現在追いかけ中の人数と未登録人数を表示する。追いかけ追加・解除後も画面上の判定へ反映する。
- 左のキャスト一覧には、キャストページKPIと同じ「現在本指名・福岡県・ランクS/A/B」の顧客人数を常時表示する。人数を押すと、そのキャストページの顧客タブを既存オーバーレイで直接開く。
- 顧客人数は全顧客の重い集計ビューを使わず、必要な条件・2列だけを管理者APIで一括取得する。選択中キャストの詳細集計と既存3一覧の定義は変更しない。
- 黒服・オーナー限定の認可を維持し、DB・migration・RLSへの変更なし。
- hotfix: 左一覧の人数をランク不問の福岡本指名全員ではなく、キャストページに表示するKPI「顧客数」と同じS/A/B限定へ修正する。優先課題の期間実来店人数は変更しない。

## v0.3.97 — フリー配席入力と月間一覧（2026-09-04）

- 日次売上入力に「本日のフリー配席数」を追加し、キャスト×営業日ごとに0〜999席を1件だけ保存する。同じ日を再保存した場合は上書きし、二重計上しない。
- フリー配席数は `cast_daily_free_seatings` で顧客来店・場内延長とは分離して保存する。入力は既存の「売上.入力」権限、閲覧は黒服・オーナーだけにRLSで制限し、キャスト本人には公開しない。
- 課題見える化シートに「キャスト別／月間一覧」のページ切替を追加する。月間一覧は選択した月について全在籍キャストを層別に1行ずつ比較する。
- 月間一覧には、実売上、設定売上、達成率、本指名本数、場内本数、フリー配席数、ボウズ数、連続ボウズ、出勤日数、設定出勤日数、残り必要出勤日数を表示する。
- 実売上は顧客の実来店売上＋場内延長売上。本指名本数は指名スナップショット（v0.3.99以降は実績保存時点）、場内本数は対象月の `nomination_history.new_status='場内'`、ボウズは既存の「確定出勤日に本指名実来店0件」定義を再利用する。
- 現在月はJSTの本日まで、過去月は月末までを集計し、未来月はUIとAPIの両方で拒否する。月間一覧は開いた時だけ専用APIから遅延取得し、従来のキャスト別画面の初期表示を重くしない。
- 適用順序は migration → verification SQL（すべてtrue／0確認）→ deploy。新画面と日次売上入力は追加テーブルを読むため順序を厳守する。

## v0.3.98 — 月間一覧の金額・可読性改善（2026-09-04）

- 課題見える化シートの月間一覧では、実売上と設定売上を「万円」に丸めず、円単位のカンマ区切りで全額表示する。
- 店舗月間合計とキャスト別一覧の文字サイズ・行高・金額列幅を拡大し、PC会議画面で読み取りやすくする。
- 横幅が足りない画面では既存どおり表内を横スクロールさせ、金額の省略や列崩れを起こさない。
- 集計値、権限、API、DB、migration、RLSは変更しない。

## v0.3.99 — 本指名スナップショット・ボウズ判定hotfix（2026-09-04）

- 実績来店の指名状況は、来店日時の履歴推測ではなく「日別売上へ保存した時点」の顧客状態を固定する。
- 場内から本指名へ変更後に売上入力した来店が場内扱いとなり、ボウズ・本指名本数を誤集計する問題を修正する。
- v0.3.92導入後の実績について、保存時刻の指名履歴と異なるスナップショットを一度だけ補正する。
- 適用順序は migration → verification（missing / invalid / mismatch / grants がすべて0）→ deploy。

## v0.3.100 — 月間一覧の直近4週顧客人数・並び替え（2026-09-05）

- 課題見える化シートの月間一覧に、各キャストの「直近4週・福岡本指名人数」を追加する。
- 今日を含む過去28日間に、本指名として実来店した福岡県のお客様を顧客ID単位で集計する。同じお客様が複数回来店しても1人と数え、予定来店・県外・期間外は除外する。
- この人数は選択中の月に関係なく常に今日基準の直近28日を表示し、既存の月間売上・本数・出勤等は選択月基準のまま変更しない。
- 月間一覧の全数値項目を、キャスト層を維持したまま各層内で「数字が高い順／低い順」に並び替えられるようにする。同値時は従来の登録順を維持する。
- API・DB・migration・RLSへの変更なし。

## v0.3.101 — AM4時の営業日切替（2026-09-08）

- 深夜営業の入力日付は、JSTのAM4:00に営業日を切り替える。0:00〜3:59に開いた入力は前日、4:00以降は当日を初期値にする。
- 対象は日別売上、顧客の実来店、場内延長・場内来店、フリー配席、顧客メモ、連絡記録、MTログ。これらは各画面で既存の日付項目を手動変更できる。
- 誕生日、入店日、来店予定、集計上の「今日」、月間集計などの通常日付は従来どおりJST 0:00切替を維持する。
- 共通関数 `businessDateJST` に切替時刻を集約し、3:59／4:00および月・年境界をテストで固定する。
- API・DB・migration・RLSへの変更なし。

## v0.3.102 — 新人教育スケジュール表（2026-09-12）

- 黒服・オーナー専用の「新人教育スケジュール表」を追加する。名称にかかわらず、対象は新人層だけでなく全層の在籍キャストとする。
- 当月カレンダーを基本に、キャストの「出勤／来客出勤」の確定日だけへ教育・面談予定を設定する。1キャスト・1日につき1件とし、再保存は上書きする。
- 区分はフェーズ1・フェーズ2・フェーズ3・コミュニケーションの4種。区分別の定型項目、自由メモ、担当する黒服・オーナー、実施済みチェックを保存する。
- キャスト別表示に加え、層別まとめ表示では同じ日に出勤する複数キャストを選び、同じ予定を一括設定できる。個別編集・一括削除にも対応する。
- APIは `requireAdmin()` とユーザーセッションRLSを併用し、DBもadmin専用4ポリシーと整合性triggerで防御する。キャスト本人にはページデータ・API・テーブルを公開しない。
- 適用順序は migration → verification SQL（すべてtrue／0確認）→ deploy。新画面は追加テーブルを読むため順序を厳守する。

## v0.3.103 — 希望出勤日の教育予定（2026-09-12）

- 新人教育スケジュール表の設定対象を「出勤／来客出勤」から「出勤／来客出勤／希望出勤」へ広げる。希望休み・休み・未定は引き続き対象外。
- キャスト別カレンダーでは「希望出勤」バッジ、層別まとめでは希望出勤の人数を表示し、確定出勤と区別できるようにする。
- 取得API・保存前のAPI検証・DB triggerの3層を同じステータス定義に揃える。管理者専用RLSや担当者検証は変更しない。
- 適用順序は migration → verification SQL（すべてtrue／0確認）→ deploy。DB triggerの更新が必要なため migration を先行する。

## v0.3.104 — 教育予定の編集・キャストMT・当日出力（2026-09-12）

- 教育予定の編集導線を明確にし、登録済み予定をキャスト単位または選択中の複数キャスト単位で削除できるようにする。
- 定型項目を、フェーズ1はSTEP1〜3の動画視聴、フェーズ2はSTEP5〜6の前日確認、フェーズ3はSTEP1〜7実戦の振り返りと完成度へ更新する。コミュニケーションの既存項目は維持し、保存済みの旧フェーズ項目も表示・再保存できる後方互換を保つ。
- 区分へ「キャストMT」を追加し、定型項目は「課題共有」「顧客確認」とする。
- 層別カレンダーの日付セルから「未設定」表示を外し、予定が入っているキャスト名を表示する。編集ドロワー内の未設定状況は一括設定に必要なため維持する。
- 日付を選んで「当日スケジュール出力」を開くと、層別にキャスト名・シフト・区分・項目・担当者・自由メモ・実施状況を並べたA4横向きの印刷専用画面を表示する。ブラウザの印刷画面からそのまま印刷またはPDF保存できる。
- 印刷画面も黒服・オーナー専用とし、既存の管理者APIから対象月を取得して指定日の予定だけを表示する。キャストにはデータを取得・表示しない。
- 適用順序は migration → verification SQL（すべてtrue／0確認）→ deploy。新アプリが `cast_mt` を保存するため、category CHECKの拡張を先行する。

## v0.3.105 — 本指名実績・ボウズ・日別カレンダーの整合（2026-09-12）

- 本指名本数・ボウズ判定・キャストページの日別カレンダーは、`customer_visits.nomination_status_at_visit` を共通の正解データとして使用する。スナップショットが無い旧行だけ顧客の現在値で補完する。
- 実来店を場内として保存した直後10分以内に「場内→本指名」へ変更した場合は、同じ登録作業として直前に保存された実来店1件だけを本指名へ補正する。現在本指名だからという理由で過去の全来店を遡及変更しない。
- v0.3.99以降に発生済みの同条件データもmigrationで一度だけ補正する。
- キャストページの日別カレンダーから予定来店を除外し、場内の実来店と同じ顧客の初回来店補完を、IDの数値／文字列差にかかわらず二重計上しない。
- 適用順序は migration → verification SQL（すべてtrue／0確認）→ deploy。migrationの補正後に本指名本数・ボウズ・日別カレンダーが同じ指名区分を参照する。

## v0.3.106 — 月間進捗差・固定見出し（2026-09-12）

- 課題見える化シートの月間一覧に、選択月の全日数に対する「月の経過率」を追加する。当月はJSTの本日を含む日数、過去月は月末の100%で計算する。
- 各キャストと店舗合計に「達成率 − 月の経過率」を表示し、プラスを計画より先行、マイナスを遅れとする。設定売上が未設定の場合は差を判定しない。
- 月間一覧のPC表示領域と表の最小幅を拡大し、金額・指標を途切れさせず表示する。
- 表は縦横スクロール領域とし、項目見出しを上端、キャスト名を左端に固定する。会議モードと通常表示の両方で同じ操作にする。
- 月の経過率と達成率との差も、既存の層内並び替えで高い順／低い順を選択できる。API・DB・migration・RLSの変更はない。
