...
### ② スタッフ権限の細分化【完了 2026-05-08】
- **追加権限 (v2)**: キャスト閲覧 / お知らせ閲覧 / お知らせ投稿 / レポート出力
- **追加権限 (v3)**: シフト閲覧 / 売上閲覧 / 顧客閲覧
- 計14権限に拡張、CHECK 制約マイグレーション v3 で全部適用
- **包含チェック**: 上位権限を持っていれば自動的に下位権限もOK
  - キャスト管理 → キャスト閲覧
  - お知らせ管理 → お知らせ閲覧/投稿
  - シフト管理 → シフト閲覧
  - 売上入力 → 売上閲覧
  - 顧客編集 → 顧客閲覧
- **ロールプリセット 4種**: 観察役 / ホールスタッフ / チーフ / マネージャー（types/index.ts に定義）
- スタッフ詳細画面にプリセット適用UIを追加（追加ベース・既存権限保持）
