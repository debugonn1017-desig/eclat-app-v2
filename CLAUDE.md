...
### ① 当日場内→当日本指名 リピート転換バグ【最優先・調査中】
- 同日中の場内→本指名アップグレードが転換カウントされない
- 怪しい: `lib/diagnosis.ts`, `useCasts.getCastKPI`, `nomination_history` 記録ロジック, `NominationFunnelCard.tsx`

### ② スタッフ権限の細分化
- 現状: '顧客編集' / 'キャスト管理' / 'お知らせ管理' / 'レポート閲覧' / '顧客引継ぎ' / '売上入力' / 'シフト管理'
- キャスト管理を付けないとお知らせ等も見えないガバ紐付けの解消
- 怪しい: `lib/auth.ts`, `app/api/admin/staff/*`, `staff_permissions` の CHECK 制約
