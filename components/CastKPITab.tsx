...
export default function CastKPITab({ castId, castName, month, kpi, castTarget, workDays }: Props) {
  ...
  const achievementItems = useMemo(() => {
    ...
    const items: { label: string; current: number; target: number; unit: string; color: string }[] = [
      ...
      { label: '本指名', current: kpi.honshimeiCount, target: ct?.target_honshimei ?? 0, unit: '人', color: '#D4607A' },
      { label: '場内', current: kpi.banaCount, target: ct?.target_banai ?? 0, unit: '人', color: '#D4A76A' },
      { label: '出勤日数', current: workDays, target: ct?.target_work_days ?? 0, unit: '日', color: C.pinkLight },
      { label: '県内（福岡）', current: kpi.localCustomerCount, target: ct?.target_local_customers ?? 0, unit: '人', color: '#7BAFCC' },
      { label: '県外', current: kpi.remoteCustomerCount, target: ct?.target_remote_customers ?? 0, unit: '人', color: C.pinkMuted },
    ]
    return items
